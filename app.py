...
import google.generativeai as genai # Keeping import as it was in original snippet, but no user-facing AI implemented.
...
# This uses the Admin SDK, typically for server-side operations with privileged access
if config.FIREBASE_ADMIN_CREDENTIALS_PATH and os.path.exists(config.FIREBASE_ADMIN_CREDENTIALS_PATH):
    ...
    app.logger.warning("Firebase Admin SDK credentials file not found or path not configured. Firestore/Storage operations might be limited.")
    db_firestore = None

# --- Gemini API Setup (as per existing config, but not actively used for user-facing features) ---
if config.GEMINI_API_KEY:
    genai.configure(api_key=config.GEMINI_API_KEY)
    # model = genai.GenerativeModel('gemini-pro') # Keeping for potential future use
    app.logger.info("Gemini API configured.")
else:
    app.logger.warning("GEMINI_API_KEY not found in config.py. AI features will be disabled.")
...
def get_member_profile_pic(user_id):
    ...
    if member and member['profilePhoto']:
        return url_for('static', filename=member['profilePhoto'].replace('static/', '')) # Adjusted for proper url_for
...
def get_user_from_member_id(member_id):
    db = get_db()
    user_id = db.execute('SELECT user_id FROM members WHERE id = ?', (member_id,)).fetchone()
    if user_id:
...
def inject_navbar_data():
    if current_user.is_authenticated:
        ...
        profile_photo_path = member['profilePhoto'] if member and member['profilePhoto'] else url_for('static', filename='img/default_profile.png')
...
def forgot_password_redirect():
    # This page now simply redirects to login as per the updated flow
...
def my_profile():
    ...
    # Fetch temporary video status if available and active
    temp_video = db.execute(
        "SELECT * FROM statuses WHERE user_id = ? AND expires_at > ? ORDER BY upload_time DESC LIMIT 1",
        (current_user.id, datetime.now(timezone.utc))
    ).fetchone()

    return render_template('my_profile.html', member=member, temp_video=temp_video, unique_key=user_details['unique_key'] if user_details else None)
    ...
    if request.method == 'POST':
        ...
        if profile_photo_file and profile_photo_file.filename != '':
            ...
            if not profilePhoto_path:
                ...
                return render_template('add_my_details.html', form_data=form_data, member=member)
...
def profile(username):
    ...
    # Fetch temporary video status if available and active for this user
    temp_video = db.execute(
        "SELECT * FROM statuses WHERE user_id = ? AND expires_at > ? ORDER BY upload_time DESC LIMIT 1",
        (profile_user['id'], datetime.now(timezone.utc))
    ).fetchone()
...
def list_members():
    db = get_db()
    # Fetch all members, exclude the current user's associated member profile
    all_members = db.execute(
        "SELECT m.*, u.username FROM members m JOIN users u ON m.user_id = u.id WHERE m.user_id != ?",
        (current_user.id,)
    ).fetchall()
    return render_template('members_list.html', all_members=all_members)
...
def add_member():
    ...
    if request.method == 'POST':
        ...
        dateOfBirth = request.form['dateOfBirth']
        gender = request.form['gender']
        # For simplicity, other fields like contact, email, bio can be added later by the new member
        # if they register and link their profile.
        # For now, we are adding a "member" record, not a "user" record.

        db = get_db()
        try:
            db.execute(
                """
                INSERT INTO members (user_id, fullName, dateOfBirth, gender)
                VALUES (?, ?, ?, ?)
                """,
                (None, fullName, dateOfBirth, gender) # user_id is NULL for now, can be linked later
            )
            db.commit()
            flash(f'New family member "{fullName}" added successfully!', 'success')
            return redirect(url_for('list_members'))
        except Exception as e:
            flash(f'An error occurred while adding the member: {e}', 'danger')
            db.rollback()
...
def inbox():
    ...
    # Fetch game invitations for the current user
    game_invitations = db.execute(
        """
        SELECT gi.*, u.username, u.originalName, m.profilePhoto
    ...
    return render_template('inbox.html', game_invitations=game_invitations, conversations=conversations)


@app.route('/api/game_invitations/<int:invite_id>/<action>', methods=['POST'])
@login_required
def api_handle_game_invitation(invite_id, action):
    db = get_db()
    invite = db.execute("SELECT * FROM game_invitations WHERE id = ? AND receiver_id = ? AND status = 'pending'", (invite_id, current_user.id)).fetchone()

    if not invite:
        return jsonify({'success': False, 'message': 'Game invitation not found or already handled.'})

    try:
        if action == 'accept':
            db.execute("UPDATE game_invitations SET status = 'accepted' WHERE id = ?", (invite_id,))
            # Logic to start the game or redirect to game lobby
            # For now, just mark as accepted
            message = 'Game invitation accepted! You can now join the game.'
            flash(message, 'success')
            # In a real app, you'd create a game session here and redirect.
            # Example: return jsonify({'success': True, 'message': message, 'redirect_url': url_for('play_game', game_name=invite['game_name'], game_id=invite['game_id'])})
            return jsonify({'success': True, 'message': message})
        elif action == 'decline':
            db.execute("UPDATE game_invitations SET status = 'declined' WHERE id = ?", (invite_id,))
            flash('Game invitation declined.', 'info')
            return jsonify({'success': True, 'message': 'Game invitation declined.'})
        else:
            return jsonify({'success': False, 'message': 'Invalid action.'})
        db.commit()
    except Exception as e:
        db.rollback()
        app.logger.error(f"Error handling game invitation: {e}")
        return jsonify({'success': False, 'message': 'Failed to handle game invitation.'})
...
def message_member():
    ...
    if request.method == 'POST':
        ...
        # Starting a chat will redirect to view_conversation or a send_message form.
        return redirect(url_for('view_conversation', partner_id=receiver_user_id))

    return render_template('message_member.html', available_users=users_with_pics)


@app.route('/view_conversation/<int:chat_room_id>', methods=['GET', 'POST'])
@login_required
def view_conversation(chat_room_id):
    db = get_db()
    ...
    try:
        ...
        for member in other_members:
            ...
            send_system_notification(
                member['user_id'],
                message_text,
                link=url_for('view_conversation', chat_room_id=chat_room_id),
...
def create_story():
    if request.method == 'POST':
        ...
        if camera_captured_data:
            # Data is base64. Decode and save.
        ...
        try:
            db.execute(
                """
                INSERT INTO stories (user_id, description, media_path, media_type, background_audio_path, visibility, timestamp)
                VALUES (?, ?, ?, ?, ?, ?, ?)
                """,
                (current_user.id, description, media_path, media_type, background_audio_path, visibility, datetime.now(timezone.utc))
            )
            db.commit()
...
def dashboard():
    db = get_db()
    user_member_profile = current_user.get_member_profile()
    if user_member_profile:
        user_member_profile_dict = dict(user_member_profile)
        user_member_profile_dict['profilePhoto'] = get_member_profile_pic(current_user.id)
    else:
        user_member_profile_dict = None

    # Fetch other users' member profiles for display
    other_users_members = db.execute(
        """
        SELECT m.*, u.username
        FROM members m
        JOIN users u ON m.user_id = u.id
        WHERE m.user_id != ?
        LIMIT 5 -- Limit to a few for dashboard overview
        """,
        (current_user.id,)
    ).fetchall()

    display_other_users_members = []
    for member in other_users_members:
        member_dict = dict(member)
        member_dict['profilePhoto'] = get_member_profile_pic(member_dict['user_id'])
        display_other_users_members.append(member_dict)

    return render_template('dashboard.html',
                           user_member_profile=user_member_profile_dict,
                           other_users_members=display_other_users_members)
...
def status_feed():
    db = get_db()
    # Fetch active statuses from friends
    # For simplicity, fetching all active statuses for now
    active_statuses = db.execute(
        """
        SELECT s.*, u.username, u.originalName, m.profilePhoto
        FROM statuses s
    ...
    for status in active_statuses:
        ...
        status_dict['profilePhoto'] = get_member_profile_pic(status['user_id'])
        display_statuses.append(status_dict)

    return render_template('status_feed.html', active_statuses=display_statuses)


@app.route('/upload_status_video', methods=['POST'])
@login_required
def upload_status_video():
    if request.method == 'POST':
        ...
        video_path = save_uploaded_file(video_file, app.config['STORY_MEDIA_FOLDER']) # Reuse story folder for status
        if not video_path:
        ...
        db = get_db()
        try:
            # Set status to expire in 24 hours
            expires_at = datetime.now(timezone.utc) + timedelta(hours=24)
            db.execute(
                """
                INSERT INTO statuses (user_id, file_path, upload_time, expires_at)
                VALUES (?, ?, ?, ?)
                """,
                (current_user.id, video_path, datetime.now(timezone.utc), expires_at)
            )
            db.commit()
            flash('Video status uploaded successfully! It will expire in 24 hours.', 'success')
            return redirect(url_for('my_profile'))
        except Exception as e:
            flash(f'An error occurred: {e}', 'danger')
            db.rollback()
            return redirect(url_for('my_profile'))
    return redirect(url_for('my_profile')) # GET request to upload_status_video redirects to my_profile

@app.route('/admin_delete_user_status/<int:member_id>', methods=['POST'])
@login_required
def admin_delete_user_status(member_id):
    db = get_db()
    member = db.execute("SELECT user_id FROM members WHERE id = ?", (member_id,)).fetchone()
    if not member:
        flash('Member not found.', 'danger')
        return redirect(url_for('my_profile')) # Or relevant profile page

    target_user_id = member['user_id']

    # Authorization: Only admin or the profile owner can delete the status
    if not current_user.is_admin and current_user.id != target_user_id:
        flash('You are not authorized to delete this status.', 'danger')
        return redirect(url_for('my_profile')) # Or relevant profile page

    try:
        # Delete only the latest active status for the user
        db.execute(
            "DELETE FROM statuses WHERE user_id = ? AND expires_at > ? ORDER BY upload_time DESC LIMIT 1",
            (target_user_id, datetime.now(timezone.utc))
        )
        db.commit()
        flash('User video status deleted successfully.', 'success')
    except Exception as e:
    ...
    if current_user.is_admin:
        ...
        return redirect(url_for('my_profile')) # Redirect user back to their profile
...
@login_required
def game_page():
    # You might fetch a list of available games from DB here
...
def play_game(game_name):
    ...
    flash(f'Game "{game_name}" is not yet available.', 'info')
...
def account_status():
    ...
    # Fetch strikes (if you implement a separate strikes table)
    strikes = [] # Placeholder if not implemented yet

    # Check for active bans
    user_data = db.execute("SELECT ban_status, ban_ends_at, ban_reason, created_at FROM users WHERE id = ?", (current_user.id,)).fetchone()
    ...
    account_status_details = {
        ...
        'last_policy_review': datetime.now(timezone.utc) # Placeholder, ideally from user activity
    }
...
def api_send_support_message_user(chat_id):
    ...
    try:
        cursor = db.execute(
            "INSERT INTO chat_messages (chat_room_id, sender_id, content, timestamp) VALUES (?, ?, ?, ?)",
            (chat_id, current_user.id, content, datetime.now(timezone.utc))
        )
        message_id = cursor.lastrowid
        db.commit()
...
def api_admin_delete_user(user_id):
    ...
    try:
        ...
        # posts, reels, stories, statuses, reports (reporter or reported), warnings, blocked_users
...
def api_admin_post_sociafam_story():
    ...
    try:
        ...
        db.execute(
            """
            INSERT INTO stories (user_id, description, media_path, media_type, visibility, timestamp, is_sociafam_story)
            VALUES (?, ?, ?, ?, ?, ?, ?)
            """,
            (admin_user['id'], description, media_path, media_type, 'public', datetime.now(timezone.utc), 1)
        )
        db.commit()
...
def forbidden(e):
    # Handle forbidden access (e.g., non-admin trying to access admin page)
